...
import dev.kingtux.batterymonitor.swipe.LeftSwipeActionCard
import dev.kingtux.batterymonitor.swipe.swipeAction
import dev.kingtux.batterymonitor.ui.BatteryMonitorTheme
...
class MainActivity : ComponentActivity() {
    @Inject
    lateinit var devicesRef: Devices
    ...
    Column(modifier = modifier) {
        TabRow(
            ...
            Tab(selected = false, onClick = {
                devices?.resetDevices(context)
            }, text = {
                Text("Force Refresh Devices")
            })
        }
        if (selectedTab == 0) {
            DevicesAsCards(
...
fun FullTest() {
    ...
    MaterialTheme {
        ...
        MainContent(enabledDevices, disableDevices, applicationDirectory = File("test").toPath(), context = context
        )
    }
}
